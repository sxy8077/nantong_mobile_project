<template>
	<view class="border">
		...
		<view class="goclient">
			...
			<button size="mini"  class="blue" @click="addContact" v-if="power">添加联系人</button>
		</view>
		<!-- 弹窗内容 -->
...
<script>
	import { ClientContactUrl,originalUrl, contactUrl,role } from "../../../../../util/urlList.js"
	...
	export default {
		...
		methods: {
			...
			async getClientMes(){
				...
				// console.log(222, res.data.count)
				this.count = res.data.count;
				if(this.count === undefined){
					this.test = true
				}else{
					this.clientList = res.data.data.filter((item) => item.status === "1")
				}
			},
			//展示弹窗
			addContact(){
