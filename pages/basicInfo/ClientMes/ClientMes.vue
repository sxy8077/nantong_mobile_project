	...
	<view class="pages">
		...
		<view class="gary">
			...
			<view class="button">
				...
				<button class="yellow" size="mini" @click="addClient" v-if="test">新增客户</button>
			</view>
			<!-- 弹窗内容 -->
...
<script>
	import {  originalUrl,messageCUrl,role } from '../../../util/urlList.js'
	...
	export default {
		...
		methods: {
			...
			async getClientunit(){
				...
				this.clientList = [...this.clientList,...res.data.results]; //原来的加上新的
			...
			async getsearch(){
				...
				res.data.results = res.data.results.filter((item) => item.status === "1");
				//console.log(444,res.data.results)
				this.clientList = [...this.clientList,...res.data.results]
